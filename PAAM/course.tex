...
\Void{del(coloredPointer* plist, int value)}{
	\Label(restart :){
		...
		\While{!found}{
			...
			\If{mark(cur$\to$mext)}{
				\tcc{cur doit être supprimé}
				\eIf{CAS(pred, curr, pointer(cur$\to$next)) != cur}{
					goto restart\;
	...
	\Label(restart :){
		...
		\While{!found}{
			Node cur = pointer(*pred)\;
			 ...
			 \If{cur == null $||$ value < cur $\to$ value}{
			 	\eIf{CAS(pred, cur, new Node(cur, value)) != cur}{
			 		goto restart\;
			 	}{
					return\;			 	
			 	}
